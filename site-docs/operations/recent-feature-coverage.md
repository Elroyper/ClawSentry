...
| OpenClaw managed setup boundary | `clawsentry init openclaw` 默认项目 `.clawsentry.toml`；`--setup` / `--setup-openclaw` explicit opt-in | OpenClaw integration status and webhook/WS paths | [OpenClaw 集成](../integration/openclaw.md)、[CLI 命令](../cli/index.md) | covered |
