...
|-------------|----------------|--------------------------|----------|------|
| Anti-bypass Follow-up Guard | `src/clawsentry/gateway/anti_bypass_guard.py`；`src/clawsentry/tests/test_anti_bypass_guard.py`；`docs/validation/anti-bypass-followup-guard-progress-2026-04-28.md` | `CS_ANTI_BYPASS_GUARD_ENABLED=true`；decision / defer-pending SSE redacted `anti_bypass` metadata；Gateway decision path | [检测管线配置：Anti-bypass Guard](../configuration/detection-config.md#anti-bypass-guard)、[环境变量](../configuration/env-vars.md#anti-bypass-guard-env) | local-verified |
