...
!!! note "覆盖口径"
    本矩阵只记录已在当前实现中存在的功能与文档入口。v0.6.1 后配置入口以 `.clawsentry.env.example`、`clawsentry start`、`clawsentry config show --effective` 和显式 `--env-file` 为准；本地 env 文件只作为显式 runtime input 或 legacy migration surface。
