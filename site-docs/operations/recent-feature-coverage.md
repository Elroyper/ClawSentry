...
!!! note "覆盖口径"
    本矩阵只记录已在当前实现中存在的功能与文档入口。v0.6.0 后配置入口仍以 `clawsentry start`、`clawsentry config wizard`、`config show --effective` 和场景模板为准；wizard 只暴露真值矩阵确认 runtime-effective 或明确标注边界的设置。
