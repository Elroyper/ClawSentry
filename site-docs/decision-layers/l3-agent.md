...
!!! note "当前实现边界（slice 4）"
    本页描述的是当前已落地的 L3 运行时语义。当前版本已经完成 `slice 2` 的装配收紧，并在 `slice 4` 中把 `suspicious_pattern` 扩展为一组有界、可解释的序列触发器：敏感读取后外传、`read -> write -> exec -> sudo` 链、临时文件暂存后外传、侦察命令后接 `sudo` 提权，以及重复 secret harvest 后进入 archive/packaging 准备。它仍然不是独立的新调度器类型，而是“先 L2 聚合、后 L3 按需介入”的确定性触发层。

!!! note "最新 operator-facing telemetry"
    当前版本里，L3 不只把详细 trace 留在数据库里。`l3_state`、`l3_reason_code`、`trigger_detail`、compact `evidence_summary`、`budget_exhaustion_event` 和 `decision_path_io` 已经向 `clawsentry watch`、Dashboard Runtime Feed、Session Detail replay、`/report/sessions`、`/report/session/{id}`、`/report/session/{id}/risk` 等现有 surfacing 路径对齐。目标是让 operator 在不翻原始 trace 的前提下也能理解“L3 是否运行过、为什么运行/降级、保留了哪些关键证据”。
