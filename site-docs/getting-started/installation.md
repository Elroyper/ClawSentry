...
=== "Homebrew (macOS)"
    ...
    !!! warning "Homebrew tap 状态：实验性"
        Homebrew tap 目前为实验性（Experimental）。formula 骨架已就绪，但完整的 Python 依赖
        resource blocks 尚未生成。**推荐使用 pip 或 uv 安装。**
