...
<div class="cs-card" markdown>
### 2. ClawSentry 评估风险
系统先走快速规则，再按需要升级到语义分析或 L3 advisory 审查。
</div>
...
| 层级 | 作用 | 什么时候触发 |
| --- | --- | --- |
| L1 规则 | 快速识别明显安全/危险模式 | 大多数事件都会先经过 L1。 |
| L2 语义 | 需要理解上下文和意图时 | 命令不明显、需要语义判断时。 |
| L3 Advisory | 复杂场景需要更深证据时 | 生成 snapshot/job/review，帮助 operator 判断；advisory-only，不改写历史判决。 |
