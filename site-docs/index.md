...
默认通过 session 日志监控接入，**可选启用 Bash preflight 防护**。
...
??? info "各框架接入详情"
    ...
    === "Codex CLI"
        ...
        当前 host 阻断只覆盖已验证的 `PreToolUse(Bash)`；`PostToolUse`、`UserPromptSubmit`、`Stop`、`SessionStart` 保持异步观察，不承诺前置阻断。
...
| 指标 | 数据 |
|:---:|:---:|
| 测试用例 | **3170 passed / 7 skipped**（发布时以 CI/验证报告为准） |
