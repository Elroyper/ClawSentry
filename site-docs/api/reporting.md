...
`decision`, `session_start`, `session_risk_change`, `alert`, `session_enforcement_change`, `defer_pending`, `defer_resolved`, `post_action_finding`, `trajectory_alert`, `pattern_evolved`
...
data: {"event_id":"evt-003","session_id":"session-001","finding_type":"injection","tier":"warn","score":0.75,"details":{},"timestamp":"2026-03-23T10:33:00+00:00"}

event: trajectory_alert
data: {"session_id":"session-001","alert_message":"Suspicious recon-then-exploit pattern detected","timestamp":"2026-03-23T10:34:00+00:00"}

event: pattern_evolved
data: {"pattern_id":"evolved-001","event":"extracted","timestamp":"2026-03-23T10:35:00+00:00"}

event: defer_pending
data: {"session_id":"session-001","approval_id":"apr-001","defer_timeout_s":300,"timestamp":"2026-03-23T10:36:00+00:00"}

event: defer_resolved
data: {"session_id":"session-001","approval_id":"apr-001","resolution":"approved","timestamp":"2026-03-23T10:37:00+00:00"}
...
| `event_id` | 关联事件 ID |
| `session_id` | 会话 ID |
| `finding_type` | 发现类型：`injection`（注入）/ `data_leakage`（数据泄露）/ `obfuscation`（混淆） |
| `tier` | 严重程度：`log_only` / `warn` / `block` |
| `score` | 检测评分（0.0-1.0） |
| `details` | 详细信息 |
| `timestamp` | 事件时间 |
...
|------|------|
| `session_id` | 会话 ID |
| `alert_message` | 告警消息（如 "Suspicious recon-then-exploit pattern detected"） |
| `timestamp` | 事件时间 |
...
|------|------|
| `pattern_id` | 模式 ID |
| `event` | 事件类型：`extracted`（提取）/ `confirmed`（确认）/ `rejected`（拒绝）/ `promoted`（晋升） |
| `timestamp` | 事件时间 |
...
| `session_id` | 会话 ID |
| `approval_id` | 审批请求 ID |
| `resolution` | 结果：`approved`（批准）或 `denied`（拒绝） |
| `timestamp` | 事件时间 |
...
curl -X POST http://127.0.0.1:8080/ahp/patterns/confirm \
  -H "Authorization: Bearer $CS_AUTH_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"pattern_id": "evolved-001", "confirmed": true}'

# 拒绝模式
curl -X POST http://127.0.0.1:8080/ahp/patterns/confirm \
  -H "Authorization: Bearer $CS_AUTH_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"pattern_id": "evolved-001", "confirmed": false}'
```
