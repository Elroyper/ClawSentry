...
from clawsentry.gateway.models import CanonicalEvent, DecisionContext, RiskLevel, RiskSnapshot
...
class L3TriggerPolicy:
    ...
    def _explicit_trigger_metadata(self, context: DecisionContext | None) -> dict[str, Any] | None:
        ...
        if summary.get("l2_escalation_requested") is True:
            # L2 semantic analysis explicitly asked for a deeper read-only
            # review (should_escalate_l3 / uncertainty on a contextual route).
            # Budget-gated upstream by CompositeAnalyzer; not a manual flag.
            return {
