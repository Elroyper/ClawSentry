...
from clawsentry.gateway.models import adapter_effect_result_summary, decision_effect_summary, utc_now_iso
...
def _is_fspr_hard_block_snapshot(snapshot: dict[str, Any]) -> bool:
    authority_reasons = {str(item) for item in snapshot.get("l1_authority_reasons") or []}
    rule_hits = {str(item) for item in snapshot.get("rule_hits") or []}
    if "fspr_package_review" in authority_reasons:
        return True
    if "first_use_skill_package_inconsistent" in rule_hits:
        return True
    for intent in snapshot.get("routing_intents") or []:
        if not isinstance(intent, dict):
            continue
        if (
            intent.get("source") == "fspr_package_review"
            and intent.get("policy_action") == "block"
            and intent.get("decision_affecting") is True
        ):
            return True
    return False
        ...
        *,
        ...
        if session is None:
            session = {
                ...
                "prior_fspr_hard_block": False,
                "prior_fspr_hard_block_event_id": "",
                "quarantine": None,
        ...
        session.setdefault("blocked_skill_lineage_facts", [])
        session.setdefault("prior_fspr_hard_block", False)
        session.setdefault("prior_fspr_hard_block_event_id", "")
        ...
        if decision_verdict == "block":
            if _is_fspr_hard_block_snapshot(snapshot):
                session["prior_fspr_hard_block"] = True
                session["prior_fspr_hard_block_event_id"] = str(event.get("event_id") or "")
