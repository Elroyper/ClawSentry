...
from clawsentry.gateway.analysis.semantic_analyzer import _compact_prompt_text
...
def _build_enterprise_llm_provider():
    ...
    if settings.normalized_provider == "openai":
        return OpenAIProvider(LLMProviderConfig(**config_kwargs))
...
async def _classify_with_llm(observation: dict[str, Any]) -> dict[str, Any]:
    ...
    settings = resolve_llm_settings()
    timeout_ms = settings.provider_timeout_ms if settings is not None else 3000.0
