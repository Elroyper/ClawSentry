...
class LLMProviderConfig:
    ...
    retry_statuses: tuple[int, ...] = (502,)
    retry_backoff_ms: int = 0
    retry_jitter_ms: int = 0
    retry_min_remaining_ms: int = 0


_T = TypeVar("_T")


def _sdk_http_client_without_env_proxy() -> object:
    """Create an SDK-compatible async HTTP client isolated from shell proxies."""
...
def _is_retryable_provider_error(exc: BaseException, statuses: tuple[int, ...]) -> bool:
    status = _provider_status_code(exc)
    return status is not None and status in set(statuses)


def _retry_delay_seconds(config: LLMProviderConfig) -> float:
    base_ms = max(0, int(config.retry_backoff_ms))
    jitter_ms = max(0, int(config.retry_jitter_ms))
...
async def _with_provider_retries(
    ...
    *,
    ...
    max_attempts = max(1, int(config.retry_max_attempts))
    min_remaining_ms = max(0, int(config.retry_min_remaining_ms))
    statuses = tuple(int(status) for status in config.retry_statuses)
    loop = asyncio.get_running_loop()
    deadline = loop.time() + max(float(timeout_ms), 1.0) / 1000.0
    attempt = 1
    while True:
        ...
        try:
            return await asyncio.wait_for(operation(), timeout=remaining_s)
        except asyncio.TimeoutError:
            raise
        except Exception as exc:
            if attempt >= max_attempts or not _is_retryable_provider_error(exc, statuses):
                raise
            delay_s = _retry_delay_seconds(config)
            remaining_before_delay_s = deadline - loop.time()
            if delay_s >= remaining_before_delay_s:
                raise
            remaining_after_delay_ms = (remaining_before_delay_s - delay_s) * 1000.0
            if remaining_after_delay_ms <= 0:
                raise
            if remaining_after_delay_ms < min_remaining_ms:
                raise
            if delay_s > 0:
...
class AnthropicProvider:
    ...
    def _get_client(self) -> object:
        ...
        if self._client is None:
            ...
            kwargs["http_client"] = _sdk_http_client_without_env_proxy()
    ...
    def _extra_body(self) -> dict[str, object] | None:
        """Compatibility body for local Anthropic-compatible endpoints."""

        normalized_model = self._model.strip().lower()
        if "minimax" in normalized_model or "kimi-k2." in normalized_model:
            return {
    ...
    async def complete(
        ...
        async def request_once() -> object:
            return await client.messages.create(**request_kwargs)  # type: ignore[union-attr]
...
class OpenAIProvider:
    ...
    async def complete(
        ...
        async def request_once() -> object:
            return await client.chat.completions.create(**request_kwargs)  # type: ignore[union-attr]
