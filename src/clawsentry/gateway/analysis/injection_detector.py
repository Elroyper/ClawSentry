...
Design basis: internal E4 phase-1 design, section 2.2
"""
...
from clawsentry.gateway.runtime.text_utils import count_invisible_chars, normalize_text
...
WEAK_INJECTION_PATTERNS: list[re.Pattern] = [
    re.compile(p, re.IGNORECASE) for p in [
        ...
        r"###\s*Instruction:",
        r"\b(must|should)\b[^\n]*\b(now|immediately)\b",
        # --- Indirect injection via tool returns (AgentDojo-style social engineering) ---
        # Content embedded in tool output that addresses the model/assistant directly
        r"(?:message|instructions?|note|request)\s+(?:from\s+[^\n]{1,60}?\s+)?to\s+you,?\s*(?:GPT|Claude|Gemini|AI|assistant|model|agent|LLM)",
        r"\byou,?\s+(?:the\s+)?(?:AI|assistant|model|agent|LLM)\b[^\n]{0,40}\b(?:must|should|need|please)\b",
        # Task-hijack preamble: "before you can solve/continue the task ... do the following (first)"
        r"before\s+you\s+(?:can\s+)?(?:solve|continue|proceed|complete|finish|answer)[^\n]{0,80}(?:do|complete|perform|execute)\s+the\s+following",
        r"(?:do|complete|perform|execute)\s+the\s+following\s+(?:task\s+|steps?\s+)?first\b",
        r"after\s+you\s+do\s+(?:that|this),?\s+you\s+can\s+(?:solve|continue|proceed|complete|finish)",
...
# Serialized tool outputs (YAML/JSON dumps) carry literal "\n" escapes and
# backslash line-continuations that split sentences mid-pattern. A collapsed
# view turns those artifacts into single spaces so patterns can match across
# them; patterns are OR-ed over both views.
_ESCAPE_ARTIFACT_RE = re.compile(r"(?:\\+\r?\n\s*\\?|\\[nrt])+")
_WS_RUN_RE = re.compile(r"\s+")


def _collapse_escaped_whitespace(text: str) -> str:
    collapsed = _ESCAPE_ARTIFACT_RE.sub(" ", text)
    return _WS_RUN_RE.sub(" ", collapsed)


def _search_any(pattern: re.Pattern, normalized: str, collapsed: Optional[str]) -> bool:
    if pattern.search(normalized):
        return True
    return collapsed is not None and bool(pattern.search(collapsed))
...
def score_layer1(
    ...
    collapsed = _collapse_escaped_whitespace(normalized)
    if collapsed == normalized:
        collapsed = None

    # Weak patterns: +0.3 each, max 1.5
    weak_count = sum(
        1 for p in WEAK_INJECTION_PATTERNS if _search_any(p, normalized, collapsed)
    )
    score += min(weak_count * 0.3, 1.5)

    # Strong patterns: +0.8 each, max 2.4
    strong_count = sum(
        1 for p in STRONG_INJECTION_PATTERNS if _search_any(p, normalized, collapsed)
    )
    ...
    if tool_name:
        ...
        tool_count = sum(1 for p in tool_pats if _search_any(p, normalized, collapsed))
