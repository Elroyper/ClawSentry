...
def test_l3_advisory_provider_smoke_skips_without_explicit_opt_in() -> None:
    ...
    assert "CS_L3_ADVISORY_PROVIDER_ENABLED" in str(result.failure_reason)
...
def test_l3_advisory_provider_smoke_strips_proxy_env_by_default() -> None:
    ...
    assert env["CS_L3_ADVISORY_PROVIDER_ENABLED"] == "true"


def test_l3_advisory_provider_smoke_script_outputs_json_when_skipped() -> None:
    ...
    env = os.environ.copy()
    for key in list(env):
        if key.startswith("CS_L3_ADVISORY_") or key in {"OPENAI_API_KEY", "ANTHROPIC_API_KEY"}:
            env.pop(key, None)
    ...
    assert "CS_L3_ADVISORY_PROVIDER_ENABLED" in payload["failure_reason"]
...
def test_l3_advisory_provider_real_network_smoke_is_explicit_opt_in(tmp_path) -> None:
    ...
    required = [
        "CS_L3_ADVISORY_PROVIDER",
        "CS_L3_ADVISORY_MODEL",
    ]
    missing = [key for key in required if not os.environ.get(key, "").strip()]
    provider = os.environ.get("CS_L3_ADVISORY_PROVIDER", "").strip().lower()
    if provider == "openai" and not (
        os.environ.get("CS_L3_ADVISORY_API_KEY") or os.environ.get("OPENAI_API_KEY")
    ):
        missing.append("OPENAI_API_KEY or CS_L3_ADVISORY_API_KEY")
    if provider == "anthropic" and not (
        os.environ.get("CS_L3_ADVISORY_API_KEY") or os.environ.get("ANTHROPIC_API_KEY")
    ):
        missing.append("ANTHROPIC_API_KEY or CS_L3_ADVISORY_API_KEY")
    if provider not in {"openai", "anthropic"}:
        missing.append("CS_L3_ADVISORY_PROVIDER=openai|anthropic")
    if missing:
    ...
    env = os.environ.copy()
    env["CS_L3_ADVISORY_PROVIDER_ENABLED"] = "true"
    env["CS_L3_ADVISORY_PROVIDER_DRY_RUN"] = "false"
