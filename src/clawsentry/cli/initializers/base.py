...
class EnvDisableResult:
    """Result of disabling one framework in a env file."""
...
def write_env_file(path: Path, header: str, values: dict[str, str]) -> None:
    """Write values using the KEY=VALUE env-file format."""
    lines = [header]
...
def merge_env_file(
    ...
    *,
    ...
    """Create or merge a env file without rotating existing shared secrets.
...
def framework_env_defaults(framework: str) -> dict[str, str]:
    """Return non-secret env-first framework activation suggestions."""
    return {"CS_FRAMEWORK": framework, "CS_ENABLED_FRAMEWORKS": framework}
...
def merge_project_framework_config(
    ...
    *,
    ...
    """Compatibility helper: suggest env-first activation without writing files."""
    _ = (target_dir, force)
    return target_dir / ".clawsentry.env.example", framework_env_defaults(framework)
...
def disable_framework_env(
    ...
    *,
    ...
    """Disable one framework in a env file without touching shared secrets."""
    if not path.exists():
